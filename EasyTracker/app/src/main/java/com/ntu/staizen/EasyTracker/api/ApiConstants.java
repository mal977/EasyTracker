package com.ntu.staizen.EasyTracker.api;

/**
 * Created by Malcom Teh on 18/9/2020.
 * Any question on how to use, ask Malcom
 */

public class ApiConstants {

    public static final String API_KEY = "";
    public static final String API_URL = "";
    public static final long API_DELAY_IN_MILLI = 10000;

    // we have to use a valid base URL even if we override it afterwards, so here it is
    public static final String baseURL = "https://api.sunrise-sunset.org/";

    public static final String PLANT_TOKEN = "Nk12U0ttU1I0U2RxWW4rcDRyU2NJQT09";
//
    public static final String SUNLIGHT_URL = "https://api.sunrise-sunset.org/";


}
